...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useEffect(() => {
    ...
    geoWidthRef.current = geometry.boundingBox?.max.x && geometry.boundingBox?.min.x
...
function Lens({ modeProps, ...p }) {
  const geometry = useMemo(() => new THREE.CylinderGeometry(1.2, 1.2, 0.3, 64, 1, true), []);
...
function Bar({ modeProps = {}, ...p }) {
  ...
  const geometry = useMemo(() => new THREE.BoxGeometry(3, 0.4, 0.4), []);
